<#
.Description
This checks to see if there are any searches taking a long time to complete (60 seconds +).
Generally this would be in a playbook is running too long due to a ton of search arguments.
...
$searchResults = Invoke-RestMethod -Uri "https://$ip:9200/_tasks?actions=*search&detailed" -Headers $header -ContentType "application/json" -UseBasicParsing



$listOfProps = ($searchResults.nodes."[nodename]".tasks[0] | gm).Name

foreach ($name in $listOfProps){


$seconds = $searchResults.nodes."[nodename]".tasks[0].$name.running_time_in_nanos/1000000000
if ($seconds -gt 60){
$seconds
Write-Host $searchResults.nodes."[nodename]".tasks[0].$name.description

Send-MailMessage -To $toEmail -From "fromemail" -Subject "Long Search in Elastic" -Body "$($searchResults.nodes."[nodename]".tasks[0].$name.description)" -SmtpServer "serverip"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUcDEYkz0MylUO1uznL7bXoaeM
# gO2ggiGyMIIDJzCCAg+gAwIBAgIQa066y01m+bBONwK13T0+hTANBgkqhkiG9w0B
...
# BgorBgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUTbZ4
# axZ80rY8ZgxrAJbb7JkLU2gwDQYJKoZIhvcNAQEBBQAEggIAnIag1287p7R76/vu
# +/CXvvALbyUQ6PyX6WpAwmGOGR2btI9iemn/akmmEf2a4iCy6j2OYmnjEgAITYWX
# odeBoakEkuaUhTd5hfsFpw0LI7thrpduqTSqAblb5QibQRfc8YEfYjYlW23kal9O
# HXJwZ5MRzIMICOiq1iO1TkDe3/ahTdV2ruwZj5PbtEJwp2cc3ltOgh8qPlnmnrQd
# yd/4Y/U3fI92SL5IpYgCPdIb53dcoTe7eruPFMxQDVf+Sj3IKholkGTkdAEHpBvL
# hb65QUVZ7jFjn/+EOkDMPF8Cr72KPyKlt+rAqyzAQ63qwurABo8Wijm6uiQB/IbD
# HE7yPIch7WzgBaNxca2HcBD7okCC646HNOK3+12EBGMtgvULTmLArLqeuZMdYxVA
# HanlrD5hayYxY8qCDcdq/rP3xtaTv8aJk9Vzt/U8PUkmy9PxjXwa5NATMkfC7b2f
# U9E08xSRONjLxq21RA17w/dh8x2n8RAOWfQekQQvAKLEcxs7SnJCQy1MSdwPMSBF
# MyTWgC2zZsHjj6+ZVsR7iI0P92XRvzAJg1HTMyU9NVAR5OCViH6/Wg0DXxN9KYCA
# EyF2FUihhjxc9OaJnfKw8U+2BrPx6hNjltH/qHHoqmTsGheJghlYdfGGI9zFcDyf
# MFKlFzp6RDB3OozvXGC2Ib++2nuhggNsMIIDaAYJKoZIhvcNAQkGMYIDWTCCA1UC
...
# AzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIzMDUwNDIxMjA0OVowKwYJ
# KoZIhvcNAQk0MR4wHDALBglghkgBZQMEAgGhDQYJKoZIhvcNAQELBQAwLwYJKoZI
# hvcNAQkEMSIEIDvonGNzSU+1r4Ijx+JEONJB8kJimokA50vuWgI5OfIPMIGkBgsq
...
# EAFIkD3CirynoRlNDBxXuCkwDQYJKoZIhvcNAQELBQAEggGAb6EZLjVAmdklh4Ep
# 3hAsgk0JOqW+tT53bsxzGpGRhqoYHpAmub1faYa2h0gHTnv+3iSDK8L8Iwg/Woq5
# grzQU0FYzbzVy4hiT6dgAl4OLE1xPMIMG7tvS7Pgw2lAloFHesBjUm9lq/ciFyIo
# rkOYFROj/Imu5Jc5KInwvchxUDERSbrOaBZuoYRxL/YW922fG5Hhb77idNT+f2qd
# bCxSdSXXBGAZrIeaQcdJjNurOTL+pP37Hw0Cu8FDNXoRkCc7WoH6tNd86WQceiup
# 95H0PvUKxfuDYoGUcqh30YVJS6dFTe9wxqDBDQjlMibqJeaBElAAyRIOs1YxtRAt
# pF6ALoYIVadJBwJFKhW9tKVkT6tR2hZLiVMUmy39/clo/zgqKoLuEW0m/Ct8XXNq
# nqzJb9S0Cbh4d1WU360dy00GDTXs5irFw+wmMQUgtclNxIBSWT7D1CTD2xda+QEd
# KA7vWKd8eKtLQnp33arYcGZ0lC1+vf+KfhQd56NSTs0zzW5m
