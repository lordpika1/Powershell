...
$enableBody = @"
    {
      "query": {
        "bool": {
          "must": [
             {
             "match_phrase": {"event.action": "Enable account."}
             },
             {
...
$scroll = $false
$searchResults = Invoke-RestMethod -Uri "https://$ip:9200/so-o365-$year.*/_search?size=10000&scroll=1m" -Headers $header -Method post -Body $enableBody -ContentType "application/json" -UseBasicParsing
...
#Generally if an account is diabled by someone it'll attach their account to the event but depenind on which Azure dashboard you do it from it may show
#as service principal. I chose to skip alerting on it for now. 

if ($($result._source.o365.audit.UserId) -match "ServicePrincipal*"){continue}
...
$($result._source.o365.audit.ObjectId) was enabled by $($result._source.o365.audit.UserId) at $eventTime
...
$disableBody = @"
    {
      "query": {
        "bool": {
          "must": [
             {
             "match_phrase": {"event.action": "Disable account."}
             },
             {
             "match_phrase": {"event.dataset.keyword": "o365.audit"}
             }
...
$scroll = $false
$searchResults = Invoke-RestMethod -Uri "https://$ip:9200/so-o365-$year.*/_search?size=10000&scroll=1m" -Headers $header -Method post -Body $disableBody -ContentType "application/json" -UseBasicParsing
$scrollID = $searchResults._scroll_id
do{
...
$($result._source.o365.audit.ObjectId) was disabled by $($result._source.o365.audit.UserId) at $eventTime
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUr5CbtUco69fApBgKiqdR5xsd
# 3PuggiGyMIIDJzCCAg+gAwIBAgIQa066y01m+bBONwK13T0+hTANBgkqhkiG9w0B
...
# BgorBgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQU9bbX
# 7wMyrF78sy0WQIWUBMVlP9EwDQYJKoZIhvcNAQEBBQAEggIAgzKsbRE3LiVkoRxs
# hCE6CbFeAXbXa49SGI/09YkMeMwJ5GFbpTFurrKNefG9wwgbh6xy58okIePbGLGn
# 5EpLvJI/J7Ulh05YPM9W3sG6f7p30y0kRoIyV7Hswtk42ylvJj9efbjL8f4stw8N
# ImMyF8RnGWus4hAK7TMbF96uAc2xzxwa5K0dYAytA1vb4J9AmDFw8WX0p1FRmIW9
# q9DZJH/6ytdHWm3Mp5t5nFva47qYXggYoCwcI/vAOXJD010IdVFdMTCmj9Ugc4Lf
# mIMj//HYfACRAxPZ7Llbg3FPO/h1USrhx9XGZmdP21neiVeBZ++A2+OzBJj2QzVd
# y3XS0vxb0YbgXmBl6AaxeI6ZYw3327gOd2ZncWdbJtKugdZIJdBLSZD3KuAdIndp
# ylTjg6jRQm7vxnhLmbVwYA3ZVSYkgqG5/zmDrZOxeqJV9LVtFJAgKzThe82qqjqe
# FFsRV75KvCe5uhicIHR5zixASAXuW/0DM4VpMQrW0aMseYzmRv0cXl4Wy+WtOHJc
# 2Mi66TIMx1N+N0mMciMQHuLTMSxnruZ6kFoo0/BJk3gPsfsHq8GkFo3HwSwK5Roz
# msVYObsHymuQkc5KjY+CcZPdXz2cdk7HWUZKNTCpHoCpS2D9WC1v1wJEln48x7jl
# 0mpNV3PIjsO2GpWED1Eb/PTMCdOhggNsMIIDaAYJKoZIhvcNAQkGMYIDWTCCA1UC
...
# AzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIzMDQwMzEzMjg1NlowKwYJ
# KoZIhvcNAQk0MR4wHDALBglghkgBZQMEAgGhDQYJKoZIhvcNAQELBQAwLwYJKoZI
# hvcNAQkEMSIEIFnga1nIW/QdN2+K0vRg4VsX/tZ6I3w96JkZQ839Eu7PMIGkBgsq
...
# EAFIkD3CirynoRlNDBxXuCkwDQYJKoZIhvcNAQELBQAEggGAKzv4sJWud0TLNGVo
# yG1ptJOdMBgWzdE6YkRsAPuoERlliSRc0rEg/bFLOCozidLsoif+ZtfEpSbkYTnT
# kAy54nH5KfnOqlBs+Z1g2HBZqqKsoW2i4yRCsz2m69ORS+41NOkCfOGA/UbD14r+
# AWGH4OXpsRZlTsapJEtlxYs6Ws2mAudoq/W2Memx6wAF6OOnCkxcinD6Od2u/HOm
# DrTTOE+yLjUNTj/o5E5grsGRhx8SI8OhX/qRbJEllnTjY1Fhfqk/LW/npM0xCNcf
# Neu5FahbDfiVBHtZWXmu3bgvFprW2N6kNMXpE2RHtHDVJ6/VYyDrChRP7j/6xpNW
# hOL37NIYyVuyDzK9cyKXx2HnTIoQEHKHMiNQGiSCZJCbqpIHoqKl9mkyTmHRW8Wm
# 5JGfvroGAXWn3Ox7IFjiABLlfLpk9FjQ8OGOYwtgOhmxFRjeRaydkni2ZmZ8Xryf
# 3nO+eW4pNd+QgV0fzNl1/A3PORd/L6/CR9+zFlpTZEuGWfL3
