<#
.Description
Removes duplicates found in the O365 index due to filebeat bug. 
#>
...
$lastRunPath = "c:\scripts\$($MyInvocation.MyCommand.Name)_lastrun"
...
foreach ($result in $searchResults.hits.hits){
    ...
    if ($($result._source.o365.audit.UserId) -match "Not Available"){
    Write-Host "skipping"

    if (!$idNATracking["$($result._source.o365.audit.Id)"]){
    #if it's not in here add it.
    #Write-Host "Adding $($result._source.o365.audit.Id),$($result._source.o365.audit.UserId)"
    #Write-Host "  "
    $idNATracking.Add("$($result._source.o365.audit.Id)",1)

    }else{

    write-host $result._source.o365.audit.Id,$result._source.o365.audit.UserId,$result._source.o365.audit.Operation,$result._source.'@timestamp'

    $invoke = Invoke-RestMethod -Uri "https://$ip:9200/$($result._index)/_doc/$($result._id)" -Method Delete -UseBasicParsing -Headers $header
...
if (!$idTracking["$($result._source.o365.audit.Id)"]){
#Write-Host "Adding $($result._source.o365.audit.Id),$($result._source.o365.audit.UserId)"
#Write-Host "  "
$idTracking.Add("$($result._source.o365.audit.Id)",1)
...
Write-Host "$($result._source.o365.audit.Id)"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUT/GMLafMzlv20DhAQelO8a1g
# pcSggiGyMIIDJzCCAg+gAwIBAgIQa066y01m+bBONwK13T0+hTANBgkqhkiG9w0B
...
# BgorBgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUjMNr
# 2faU7aACQMXHrHXC9lp37rAwDQYJKoZIhvcNAQEBBQAEggIAxBHpynflvCvzQhz0
# +AYRXK3Fu2+209xMR5FQzGWMIQ3N74BiLrk/3aWSrZu7YBqV8USam73UfrwICHrE
# qiib7k21EGqRETo5+2MWnsNoEqlRnivG7tnoklER9Y20GuO4IjuarSrEP2U06SVh
# zQ1zpKGkxRfUQEOs7OzDNer0wY+e1DGr3zNqelmM1+o+8YONZ3owh0LCOX5LZN4u
# moDtEnOHHJeXEc9OJ926Wn9+wFCtHWZOl/+o3+2JMjpAxM5V7Yu/+SGw1g2uJXGn
# BlM5eG2NlrEem//9ES0+dvQD0nbyH0kx0kM8QD++7whrEUU5S09ceQaydVLhxMb/
# C+aG5mcic85CsME8qcsoJHyfdfDPDn0XloBrSbwzhoiQw5rYFD/1RKFamCHGahK1
# 357ij4iolY+Pnxutsf0EfSl2oFDfv4kHELnxtIu6EnhwRvN76Nu15zJKhR+bwTah
# MbpU5FgkOTvhiJBZLmcJLC/hMovPLh7AE9sO9YEPs2e76tg7KGKUl5v0ge5leaWr
# zyO4lyXgyo53K6t73KmUcTTUM+KUQRNJaT/Hnb2gPvBSUyLy8zf2ZjVbiajh/Fmc
# 0Inbl3LTTOi5uX9aniaOKeeyJYZQNQEPjUWpOh9zOTnHKl535FV1svAuAx7eGI45
# bQMlNxuVNk/tOgUZ/qaOsFwa4suhggNsMIIDaAYJKoZIhvcNAQkGMYIDWTCCA1UC
...
# AzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIzMDUwNTE1MDUxMFowKwYJ
# KoZIhvcNAQk0MR4wHDALBglghkgBZQMEAgGhDQYJKoZIhvcNAQELBQAwLwYJKoZI
# hvcNAQkEMSIEIBjwhWgIb8sYLNqx66yB4XU8ODNHYuRI43fjGanEbv9UMIGkBgsq
...
# EAFIkD3CirynoRlNDBxXuCkwDQYJKoZIhvcNAQELBQAEggGAZImYEFTckAtJddPU
# urYqbRkDBZrhRlK1lSyRVGFoJBuvL84tlihwZOj3fbhegfGogUk48B2CZwDGzoHF
# ctzvKgg3A1d65pijId9CZvAZi2qvhYpljQKOHQk3rAThg6IrfFv+1amPwm7u+gEL
# TXfIs+fWjyDNEY7BJrKZisUcNsusvH50HVwyY9fK8eSMbQE6nniHhv/FDMOOJfMw
# hF3AJezPdwirnxtoN4cDH28Dp29xClSUJwR8bh7TobUIyuQ7EnFc7OTJ8ZUPC7Dc
# N9BqUl2sUpPyRLV5TRKqUvPptXSHQbYkf+6QbHV2EWqebo0PScduT/cpr4nz3JRJ
# CV2yftdiacVqag06epPRNTwbzFGlUXjzf6KgD7vju0hoYJehF45H0pHpuxnJD6zz
# mnYqYgPuGFL38+Rzw9IwnHKROXJItM4rwjXTVm0T7F4RUMRaJSYLqOyEFFTd4urx
# iZJKXMdtm47YPzssJhCq8phQTDWD9zE8o62NpePYC4RTi1AI
